...
public:
    ...
    void initializeMPC(const vpHomogeneousMatrix& cMo) {
        try {
            ...
            MPCConfig config;	//MPCConfig 控制器的说明书
            
            nh_.param("mpc_Np", config.Np, 6);
            nh_.param("mpc_Ts", config.Ts, 0.1);
            nh_.param("mpc_v_max", config.v_max, 1.0);
            nh_.param("mpc_w_max", config.w_max, 1.0);
            ...
            nh_.param("mpc_Q_scale", Q_scale, 1.0);   //越小越关注误差项
            nh_.param("mpc_R_scale", R_scale, 10000.0);	//越小越关注控制量
            ...
            mpc_controller_ = std::make_unique<MPC_IBVS_Controller>(config);	//在堆上创建MPC控制器对象,config作为构造参数
        ...
        try {
            ...
            if (cMo_vec.size() == 1) {
                ...
                if (corners.size() == 4) {
                    ...
                    Eigen::VectorXd v_c_mpc = mpc_controller_->solveMPC(p_, pd_, tau_prev_);//有崩溃嫌疑
                    tau_prev_ = v_c_mpc;
    ...
    void run() {
        ROS_INFO("Into run()");
        ROS_INFO("Starting MPC-IBVS control loop..."); //到这里没问题
