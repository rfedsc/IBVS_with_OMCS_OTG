...
int main(int argc,char **argv){
    ...
    bool opt_plot = true;
    //使用std::vector存储轨迹
    std::vector<std::vector<vpImagePoint>> traj_corners;
    //定义期望的像素坐标
    vpImagePoint ip;
    
    //设置相机外参
    //vpPoseVector ePc(-0.0312543, -0.0584638, 0.0309834, -0.00506562, -0.00293325, 0.0117901);
    vpHomogeneousMatrix eMc({-0.9851706042,0.0283381949,-0.1692212377,80.3552129423/1000,
                             -0.0304233835,-0.9994896289,0.009741633,93.2879574536/1000,
                             -0.1688588117,0.0147454531,0.9855299454,18.6885315346/1000,
                            0,0,0,1});

    //设置相机内参
    vpCameraParameters cam(598.8434,599.0183,325.2096,249.8498);
    ...
    while(ros::ok()&&!has_converged){
        ...
        if(cMo_vec.size()==1){
            ...
            if(first_time){
                ...
                //没有平移,z轴绕逆时针旋转180度
                v_oMo[0]=vpHomogeneousMatrix(0,0,0,0,0,0);
                v_oMo[1]=vpHomogeneousMatrix(0,0,0,0,0,M_PI);
                //计算两个可能相对的位姿
