...
private:
    ...
    std::vector<vpFeaturePoint> p_current_;
    std::vector<vpFeaturePoint> pd_target_;
...
private:
    ...
    // 新增约束处理函数
    void applyVelocityConstraints(Eigen::VectorXd& v_c) const;
    void applyFOVConstraints(const std::vector<vpFeaturePoint>& p_current,
                            Eigen::VectorXd& v_c) const;
    bool checkFOVViolation(const vpFeaturePoint& feature) const;
};
